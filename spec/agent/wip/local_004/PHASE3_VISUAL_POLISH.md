...
- [ ] 暗色ベース球体メッシュの実装
- [ ] `sphereBaseMode` 3 モード切替（translucent / opaque / none）
- [ ] `sphereBaseColor` と `backgroundColor` の視覚的差別化
- [ ] ノードシェーダの `colorUniform` 導入（`material.color` 非依存）
- [ ] グロー計算を外縁のみ（`dist > 0`）に制限
- [ ] `sphereGridColor` の独立制御
- [ ] カラープリセット機構（5 種: hud-cyan / emerald / amber / frost / infrared）
- [ ] プリセット GUI セレクタ + 一括適用ロジック
