...
- 実装: `THREE.SphereGeometry` + `THREE.MeshBasicMaterial`（`transparent: true`）
- デフォルト色: `#0A0F1A`（背景と同系の暗色）
- `sphereBaseVisible` で表示切替、`sphereBaseColor` で色指定、`sphereBaseOpacity` で不透明度調整
...
gridFolder.add(params, "sphereBaseVisible").name("Base Visible");
gridFolder.add(params, "sphereBaseOpacity", 0.05, 1, 0.05).name("Base Opacity");
...
| `sphereBaseColor`       | 有効な hex 文字列                     | `"#0A0F1A"`              | hex パターン一致 |
| `sphereGridColor`       | 有効な hex 文字列                     | `"#00BFFF"`              | hex パターン一致 |
| `sphereBaseOpacity`     | 0–1 の範囲                            | `0.6`                    | 範囲内           |
...
- [ ] 暗色ベース球体メッシュの実装
- [ ] `sphereBaseVisible` / `sphereBaseColor` / `sphereBaseOpacity` 制御
