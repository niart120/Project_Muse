...
Phase 1 で実装した距離閾値エッジに加え、k-NN・球面ドロネー・MST・Gabriel グラフの 4 アルゴリズムを実装し、GUI から切り替えられるようにする。エッジの描画経路として直線と測地線弧を選択可能にする。
...
| ------------------------ | --------------------- | --------------------------------------- |
| エッジアルゴリズム数     | 1（距離閾値）         | 5（距離, k-NN, Delaunay, MST, Gabriel） |
| エッジ経路描画           | 直線のみ              | 直線 / 測地線弧の切替                   |
| GUI 切替                 | なし                  | ドロップダウンでアルゴリズム選択        |
| 100 ノード時のエッジ計算 | < 1 ms （距離閾値）   | < 50 ms （最重量の Delaunay 含む）      |
...
球面上の点に Delaunay 三角形分割を適用する。3D 凸包の双対として実装できる。

- アプローチ: 球面上の点を 3D 凸包の頂点とみなし、凸包の辺 = ドロネーエッジとする。半径 1 の球面上に点があるため、原点から見た凸包がそのまま球面ドロネーになる
- 凸包計算: インクリメンタル法で自前実装するか、軽量ライブラリの導入を検討する。N ≤ 100 であれば単純な O(N²) アルゴリズムでも許容範囲内
- 計算量: O(N log N)（凸包アルゴリズム依存）
- 平面グラフ保証: あり（ドロネー三角形分割は定義上平面グラフ）
- パラメータ: なし（点の配置のみで決まる）

実装方針: N ≤ 100 の範囲では、自前のインクリメンタル凸包が現実的。外部依存を避けられる。ただし実装コストが高い場合は Phase 内でライブラリ導入を再検討する。

```typescript
export function computeDelaunayEdges(state: NodeState, params: NodeGardenParams): EdgeResult;
```
...
| ドロネー     | 保証あり   | 約 3N（三角形分割）   | なし              |
| MST          | 保証あり   | N - 1（最疎）         | なし              |
| Gabriel      | 保証あり   | ドロネー以下 MST 以上 | なし              |
...
| `computeDelaunayEdges` | 正四面体頂点                            | 4 点               | 6 エッジ（完全グラフ = ドロネー）    |
| `computeDelaunayEdges` | 8 点（立方体頂点を球面射影）            | 8 点               | エッジが交差しないことを幾何的に検証 |
...
| `computeGabrielEdges`  | ドロネーの部分グラフ                    | 20 ノード          | Gabriel エッジ ⊆ ドロネーエッジ      |
