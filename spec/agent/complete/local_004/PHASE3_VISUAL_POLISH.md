...
| 球体表示       | なし                     | 暗色ベース球体 + 緯度経度グリッド（各独立制御）               |
...
全形状で外縁にアンチエイリアス（`smoothstep`）を適用し、外縁のみ（`dist > 0`）にグロー減衰（`exp(-max(dist, 0) * glowFalloff)`）を加える。色は `colorUniform` で制御し、`colorNode` 内で RGB として適用する（`material.color` は使用しない）。
...
interface NodeGardenParams {
  ...
  /** ベース球体の表示モード */
  sphereBaseMode: "translucent" | "opaque" | "none";
  /** ベース球体の色 (hex) */
...
const phase3Defaults = {
  ...
  sphereBaseMode: "translucent" as const,
...
半径 `sphereRadius` の暗色半透明球体をシーンに配置し、ノードガーデンの土台とする。

- 実装: `THREE.SphereGeometry` + `THREE.MeshBasicMaterial`
- `sphereBaseMode` で 3 モード切替: `"translucent"`（半透過, opacity 0.6）/ `"opaque"`（不透明, opacity 1.0）/ `"none"`（非表示）
- デフォルト色: `#050810`（背景 `#0D1117` より暗い色で視覚的に区別）
- `sphereBaseColor` で色指定
- `sphereRadius` 変更時はジオメトリを再構築する
...
- 経度線: 15° 間隔の大円（12 本）
- 各線は 64 セグメントの折れ線で近似
- 色: `sphereGridColor` で独立制御（デフォルト: `#00BFFF`）、不透明度は `sphereGridOpacity`（5–10%）
...
gridFolder.add(params, "sphereBaseMode", ["translucent", "opaque", "none"]).name("Base Mode");
gridFolder.addColor(params, "sphereBaseColor").name("Base Color");
gridFolder.add(params, "sphereGridVisible").name("Grid Visible");
gridFolder.add(params, "sphereGridOpacity", 0.01, 0.2, 0.01).name("Grid Opacity");
gridFolder.addColor(params, "sphereGridColor").name("Grid Color");

const colorsFolder = gui.addFolder("Colors");
colorsFolder
  .add(presetState, "current", ["hud-cyan", "emerald", "amber", "frost", "infrared"])
  .name("Preset");
colorsFolder.addColor(params, "nodeColor").name("Node");
colorsFolder.addColor(params, "edgeColor").name("Edge");
colorsFolder.addColor(params, "backgroundColor").name("Background");
```

### 4.4 カラープリセット

local_001 のカラーパレット方針に基づき、全体の配色を一括切替できるプリセット機樫。

| プリセット名 | メイン色  | 背景      | テーマ                 |
| ------------ | --------- | --------- | ---------------------- |
| `hud-cyan`   | `#00BFFF` | `#0D1117` | デフォルト HUD シアン  |
| `emerald`    | `#00FF87` | `#0A1510` | エメラルドグリーン     |
| `amber`      | `#FFB000` | `#15100A` | 琥珀色の警告トーン     |
| `frost`      | `#B0E0FF` | `#0A0F18` | 冷青白のレーダートーン |
| `infrared`   | `#FF3333` | `#150A0A` | 赤外線イメージ         |

各プリセットは `nodeColor`, `edgeColor`, `sphereGridColor`, `sphereBaseColor`, `backgroundColor` の 5 色を定義。プリセット選択時に全色を一括適用し、GUI のカラーピッカー表示も同期する。プリセット適用後に個別色を手動変更することも可能。
...
| `sphereBaseColor`       | 有効な hex 文字列                     | `"#050810"`              | hex パターン一致 |
| `sphereBaseColor`       | `backgroundColor` と異なる            | —                        | 不一致           |
| `sphereGridColor`       | 有効な hex 文字列                     | `"#00BFFF"`              | hex パターン一致 |
| `sphereBaseMode`        | 有効な値                              | `"translucent"`          | 有効値に含まれる |     | `colorPresets` | 全プリセットの全色が有効 hex | 5 プリセット | hex パターン一致 |
| `colorPresets`          | 各プリセットの base/bg が異なる       | —                        | 不一致           |
| `colorPresets`          | hud-cyan が defaultParams と一致      | —                        | 一致             |
...
- [x] `shaders/node-shapes.ts` — SDF 形状関数（circle, cross, diamond, hexagon）
- [x] ノード描画を TSL ベースマテリアルに置き換え
- [x] `nodeShape` uniform 切替の実装
- [x] ノードグロー減衰の実装
- [x] `shaders/edge-styles.ts` — distance-fade マテリアル
- [x] pulse エッジスタイル実装
- [x] signal（パケット伝搬）エッジスタイル実装
- [x] breathing エッジスタイル実装
- [x] `edgeStyle` GUI 切替の実装
- [x] `shaders/bloom.ts` — TSL ブルームポストプロセス
- [x] ブルーム GUI コントロール
- [x] 球体グリッドオーバーレイ生成
- [x] `sphereGridVisible` 切替の実装
- [x] 暗色ベース球体メッシュの実装
- [x] `sphereBaseMode` 3 モード切替（translucent / opaque / none）
- [x] `sphereBaseColor` と `backgroundColor` の視覚的差別化
- [x] ノードシェーダの `colorUniform` 導入（`material.color` 非依存）
- [x] グロー計算を外縁のみ（`dist > 0`）に制限
- [x] `sphereGridColor` の独立制御
- [x] カラープリセット機構（5 種: hud-cyan / emerald / amber / frost / infrared）
- [x] プリセット GUI セレクタ + 一括適用ロジック
- [x] `params.ts` に Phase 3 パラメータ追加
- [x] `mod.ts` export 更新
- [x] テスト追加・実行
- [x] 目視での HUD テーマ品質確認
- [x] レビュー完了
